...
void init_simulation_extra_parameters(
    ...
    simulation->simulate_fun = with_credits ? simulate_sand_credits : simulate_sand;
...
*/
    ...
    for (int i = 0; i < simulation->source->num_sources; i++) {
        if (simulation->source->source_state[i] && simulation->source->spawned[i] < simulation->source->capacities[i] && simulation->source->timers[i] >= simulation->source->frequencies[i]) {
            ...
            float t_last = simulation->total_time;- simulation->source->timers[i];
    ...
    simulation->total_time += dt_clamped;
}
